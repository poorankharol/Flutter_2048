...
class BoardManagerTiny extends StateNotifier<Board> {
  ...
  // 14, 10, 6, 2,
  // 15, 11, 7, 3];
  final verticalOrder = [8, 4, 0, 5, 1, 6, 2, 7, 3];

  final StateNotifierProviderRef ref;

  BoardManagerTiny(this.ref) : super(Board.newGame(0, [])) {
  ...
  void load() async {
    ...
    var box = await Hive.openBox<Board>('boardBoxTiny');
  ...
  bool _inRange(index, nextIndex) {
    return index < 3 && nextIndex < 3 ||
        index >= 3 && index < 6 && nextIndex >= 3 && nextIndex < 6 ||
        index >= 6 && index < 9 && nextIndex >= 6 && nextIndex < 9 ||
        index >= 9 && nextIndex >= 9;
  }
  ...
  Tile _calculate(Tile tile, List<Tile> tiles, direction) {
    ...
    int nextIndex = ((index + 1) / 3).ceil() * 3 - (asc ? 3 : 1);
...
final boardManagerTiny = StateNotifierProvider<BoardManagerTiny, Board>((ref) {
  return BoardManagerTiny(ref);
});
